...
function FAQ({ q, a, isDark }: { q: string; a: string; isDark: boolean }) {
  return (
    <View className={`rounded-xl p-4 mb-3 ${isDark ? "bg-gray-800" : "bg-gray-50"}`}>
...
export default function SupportScreen() {
  ...
  const handleEmailSupport = () => Linking.openURL("mailto:suds.application@gmail.com?subject=Suds%20Support");
  const handleBugReport = () =>
    Linking.openURL(
      "mailto:suds.application@gmail.com?subject=Bug%20Report&body=App%20version%3A%0ADevice%3A%0AOS%20version%3A%0A%0ASteps%20to%20reproduce%3A%0A1.%20%0A2.%20%0A%0AExpected%3A%0AActual%3A",
    );
  ...
  return (
    <PublicLayout>
      <Head>
        ...
        <meta
          name="description"
          content="Get help with the Suds drink tracking app. Contact support or report a bug."
        />
      </Head>
      {/* Header */}
      <View style={{ marginBottom: 48 }}>
        <Text className={`text-4xl font-bold mb-3 ${isDark ? "text-white" : "text-gray-900"}`}>Support</Text>
      ...
      <Section title="Get in Touch" isDark={isDark}>
        <View style={{ flexDirection: "row", gap: 12, flexWrap: "wrap" }}>
          <ContactCard
            label="Email Support"
            value="suds.application@gmail.com"
            onPress={handleEmailSupport}
            isDark={isDark}
          />
          <ContactCard label="Report a Bug" value="Send bug report" onPress={handleBugReport} isDark={isDark} />
        </View>
      ...
      <Section title="Delete Your Account" isDark={isDark}>
        <View
          className={`rounded-xl p-5 border ${isDark ? "bg-gray-800 border-gray-700" : "bg-red-50 border-red-100"}`}
        >
          ...
          <Text
            ...
            style={{ lineHeight: 22 }}
          >
            Deleting your account removes your profile, drink logs, followers, and all associated data. This action is
          ...
          <Text className={`text-xs mt-4 ${isDark ? "text-gray-600" : "text-gray-400"}`}>
            Can't access the app? Email suds.application@gmail.com with the subject "Delete Account" from your

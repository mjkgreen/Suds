...
export function GoalCard({ userId, stats }: GoalCardProps) {
  ...
  return (
    <View className="bg-card mx-4 mt-4 rounded-2xl border border-gray-400 p-4">
      <View className="flex-row items-center justify-between mb-3">
        <Text className="text-xs text-gray-400 font-medium uppercase tracking-wide">Weekly Limit</Text>
        {!editing && (
          <Pressable onPress={startEdit}>
            <Text className="text-xs text-amber-500 font-semibold">{limit ? "Edit" : "Set Limit"}</Text>
          </Pressable>
        )}
      </View>
      ...
      {editing ? (
        <View className="flex-row items-center gap-3">
          ...
          />
          <Pressable className="bg-amber-400 rounded-xl px-4 py-2.5" onPress={save}>
        ...
        <View>
          <View className="flex-row items-end justify-between mb-2">
            <Text className={`text-2xl font-bold ${overLimit ? "text-red-500" : "text-gray-400"}`}>
              {weeklyCount}
        ...
        <Text className="text-gray-400 text-sm">Set a weekly drink limit to track your moderation.</Text>
      )}
    </View>
  );
}
